...
from scaffold.graph.graph.abstract import _AbstractGraph
from scaffold.graph.io import _parse_dot
from scaffold.graph.primitives.edge import WeightedEdge
...
from scaffold.graph.walk import WeightedWalk
...
class WeightedGraph[W: Weight](_AbstractGraph[WeightedEdge[W]]):
    ...
    def from_edge_list(
        ...
        *,
        ...
        for u, v, w in edges:
            g.add_edge(
                Vertex(u) if isinstance(u, str) else u, Vertex(v) if isinstance(v, str) else v, w
            )
        return g

    def to_edge_list(self) -> list[tuple[Vertex, Vertex, W]]:
        """
        간선을 ``(u, v, weight)`` 튜플 목록으로 반환한다.

        무방향·양방향 그래프에서는 각 간선을 한 번만 포함한다.
        """
        seen: set[frozenset] = set()
        result: list[tuple[Vertex, Vertex, W]] = []
        for label, edges in self._adj.items():
            for e in edges:
                if self.kind != EdgeKind.DIRECTED:
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                result.append((self._vertices[label], e.dst, e.weight))
        return result
        ...
        return _json.dumps(
            {
                ...
                "edges": [(u.label, v.label, w) for u, v, w in self.to_edge_list()],
            }
        )
