...
class UnweightedGraph(_AbstractGraph[Edge]):
    ...
    def to_edge_list(self) -> list[tuple[Vertex, Vertex]]:
        """간선을 ``(u, v)`` 튜플 목록으로 반환한다.
            ...
            for nlabel in nbrs:
                if self.kind != EdgeKind.DIRECTED:
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                result.append((self._vertices[label], self._vertices[nlabel]))
        return result
        ...
        return _json.dumps(
            {
                ...
                "edges": [(u.label, v.label) for u, v in self.to_edge_list()],
            }
        )
